...
class LS4_CCD_Map():
    ...
    def __init__(self,
        ...
        assert ls4_conf['amp_direction'] in ["both","custom","left","right"],\
           "config parameters [%s]=[%s] must be both, custom, left, or right" %\
             ('amp_direction',ls4_conf['amp_direction'])

        self.debug("ls4_conf[amp_direction]: %s" % ls4_conf['amp_direction'])

        if ls4_conf['amp_direction'] in ["left","right","custom"] :
    ...
    def _get_tap_data(self,ccd_name=None,ccd_location=None,amp_name=None,data_key=None):
        ...
        assert (amp_name is None) or (amp_name in ["LEFT","RIGHT","BOTH","CUSTOM"]),\
               "amp_name %s must be LEFT,RIGHT,BOTH,CUSTOM, or None" % amp_name
        ...
        data = info[key]
        if amp_name in ['LEFT','RIGHT','CUSTOM']:
