...
if $HELP_FLAG; then
   ...
   echo "  -custom            : Readout CCDs through custom configuration"
...
if $BOTH_AMP; then
  ...
  AMP_DIRECTION="right"
elif $CUSTOM_AMP; then
  AMP_DIRECTION="custom"
else
  echo "ERROR: amp choice is neither both, custom, left, nor right"
  exit
fi    
...
if $SUB_READOUT; then
  ...
  if $BOTH_AMP; then
    ...
    ARCHON_CFG_LIST="northeast_right.acf,southeast_left_CD_right.acf,northwest_right.acf,southwest_right.acf"
    CLEAR_TIME=60 
  fi
fi
